...
void writing_arguments(string nameVar, int value) {
    if (inFunc == 0) {
...
void writing_arguments(string nameVar, char value) {
    if (inFunc == 0) {
...
void writing_arguments(string nameVar, short value) {
    if (inFunc==0) {
...
void ParsVars(ifstream& stream, const string& typeVar) {
    ...
    if ((semicolon != -1) && (comma == -1) && (equals == -1)) {
        if (word.length() == semicolon + 1) {
            ...
            if (typeVar == "int") { writing_arguments(nameVar, int_value); }
            if (typeVar == "short") { writing_arguments(nameVar, short_value); }
            if (typeVar == "char") { writing_arguments(nameVar, char_value); }
        }
                ...
                if (word == "=") {
                    ...
                    if (semicolon != -1) {
                        if (word.length() == semicolon + 1) {
                            ...
                            endCommand = ";";
                            if (typeVar == "int") { int_value = stoi(word); writing_arguments(nameVar, int_value); };
                            if (typeVar == "short") { int orowe = stoi(word); short_value = (short)orowe; writing_arguments(nameVar, short_value); };
                            if (typeVar == "char") { char_value = word[0]; writing_arguments(nameVar, char_value); };
                        }
                        else { throw exception(); };
                    }
                    else {
